...
func main() {
	flag.Usage = func() {
		const usageFormat = "Usage: %s <language> <schema-file> <root-pulumi-supavisor-native-dir>"
	...
	case Schema:
		...
		providerDir := filepath.Join(".", "provider", "cmd", "pulumi-resource-supavisor-native")
...
func mustWritePulumiSchema(pkgSpec schema.PackageSpec, outdir string) {
	...
	// Convert required property names under resources to camelCase
	for _, resource := range pkgSpec.Resources {
		for i, req := range resource.Required {
			// Convert snake_case to camelCase
			parts := strings.Split(req, "_")
			for j := 1; j < len(parts); j++ {
				parts[j] = strings.Title(parts[j])
			}
			resource.Required[i] = strings.Join(parts, "")
		}
	}
